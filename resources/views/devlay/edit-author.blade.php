    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-5 mb-3">
                <form method="post" action="{{ route('author.update', $author->id) }}" enctype="multipart/form-data" class="flex flex-column justify-center p-4 mt-[25vh] card-body rounded-lg bg-slate-700 shadow-lg">
                    @csrf
                    @method('PUT')
                    <h1 class="flex text-slate-100 justify-center font-medium text-2xl">Edit Data Penulis</h1>
                    <label class="form-label required text-slate-200" for="nama">Required</label>
                    <input type="text" id="nama" class="form-control mb-4" 
                        @error('name')
                            is-invalid
                        @enderror name="name" placeholder="Author's Name" value="{{ $author->name }}">
                    @error('name')
